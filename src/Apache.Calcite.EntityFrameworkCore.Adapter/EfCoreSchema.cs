...
namespace Apache.Calcite.EntityFrameworkCore.Adapter
{
    ...
    public class EfCoreSchema : AbstractSchema
    {
        ...
        public static EfCoreSchema Create(string name, Func<DbContext> contextFactory)
        {
            return Create(name, new DelegateDbContextFactory(contextFactory), DefaultRexToLinqTranslatorFactory.Instance);
        }
        ...
        /// <returns>The newly created schema.</returns>
        public static EfCoreSchema Create(string name, IDbContextFactory contextFactory)
        {
            return Create(name, contextFactory, DefaultRexToLinqTranslatorFactory.Instance);
        }
        ...
        /// <returns>The newly created schema.</returns>
        public static EfCoreSchema Create(string name, Func<DbContext> contextFactory, IRexToLinqTranslatorFactory? translatorFactory)
        {
            ArgumentNullException.ThrowIfNull(contextFactory);
            translatorFactory ??= DefaultRexToLinqTranslatorFactory.Instance;
            return new EfCoreSchema(name, new DelegateDbContextFactory(contextFactory), translatorFactory);
        }
        ...
        /// <param name="translatorFactory">Optional factory that creates Rex-to-LINQ translators. Pass <see langword="null"/> to use the default.</param>
        /// <returns>The newly created schema.</returns>
        public static EfCoreSchema Create(string name, IDbContextFactory contextFactory, IRexToLinqTranslatorFactory? translatorFactory)
        {
            ArgumentNullException.ThrowIfNull(contextFactory);
            translatorFactory ??= DefaultRexToLinqTranslatorFactory.Instance;
            return new EfCoreSchema(name, contextFactory, translatorFactory);
        }
        ...
        /// </summary>
        /// <remarks>
        /// A schema does not attach itself to a parent: whoever creates it registers it, on a
        /// <c>CalciteDataSourceBuilder</c> or on the <see cref="SchemaPlus"/> Calcite hands a
        /// <see cref="EfCoreSchemaFactory"/>.
        /// </remarks>
        /// <param name="name">The name of this schema.</param>
        /// <param name="contextFactory">Factory that produces <see cref="DbContext"/> instances.</param>
