    ...
    public class EfCoreIntersect : Intersect, EfCoreRel
    {
        ...
        public System.Linq.Expressions.Expression Implement(EfCoreRelImplementor implementor, EfCoreTranslationContext rexContext)
        {
            ...
            else if (resultType.IsAssignableTo(typeof(System.Linq.IQueryable)))
            {
                ...
                {
                    throw new System.InvalidOperationException($"EfCoreIntersect input expression type {resultType.Name} implements IQueryable but not IQueryable<T>");
                }
            }
            else
            {
                throw new System.InvalidOperationException($"EfCoreIntersect input expression type {resultType.Name} is not IQueryable<T>");
            }

            for (int i = 1; i < n; i++)
            {
                var right = implementor.VisitChild((RelNode)inputs.get(i), rexContext);
                result = System.Linq.Expressions.Expression.Call(QueryableMethods.Intersect.MakeGenericMethod(elementType), result, right);
            }
